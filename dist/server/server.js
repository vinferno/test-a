...
app.post("/api/create-inventory-item", async function (req, res) {
    const { product, condition, status, location, salePrice, dateSold, } = req.body;
...
app.post("/api/user-cart", async function (req, res) {
    ...
    res.json({
        ...
        cartItems: cartItems.reduce((acc, item) => {
            console.log(acc);
            const found = acc.find((i) => i._id === item.inventoryItem.product._id);
            if (found) {
                found.quantity += 1;
            }
            else {
